...
function AllProducts() {
  ...
  return (
    ...
    <div className=' px-0 py-2  lg:px-4 lg:py-8 lg:pt-1  shadow-xl   '>
      <ToastContainer />

      <h1 className=' text-[28px] mx-2 lg:text-[40px] lg:mx-1 lg:my-1 font-semibold'>Products</h1>
      {
        isLoading ? (<Loader />)
            ...
            <div className='flex items-start gap-5 overflow-y-scroll  py-4 px-4 '>
              {products.map((product, index) => (
                <div className='bg-[#fff] h-[285px] lg:h-[370px] lg:min-w-[250px] min-w-[170px] shadow-2xl rounded-[10px] hover:transform hover:scale-105 hover:transition-transform duration-700 cursor-pointer' key={index} >
                  ...
                  <img src={product.thumbnail} alt='logo' className='bg-white h-[150px] w-[170px] lg:w-[270px] lg:h-[200px] border rounded-[10px] shadow-xl' onClick={() => handleNavigate(product.id)} />
                  <div className='p-2' onClick={() => handleNavigate(product.id)}>
                    <h1 className='text-[14px] lg:text-[20px] font-semibold w-60'>{product.title.substring(0, 18) + "..."}</h1>
                    <h1 className='text-[14px] lg:text-[16px]'>Brand: {product.brand.substring(0, 11) + "..."}</h1>
